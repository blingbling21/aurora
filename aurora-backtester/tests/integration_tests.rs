...
async fn test_backtest_result_correctness() -> Result<()> {
    ...
    let klines = vec![
        // 下跌趋势
        Kline { timestamp: 1000, open: 100.0, high: 101.0, low: 99.0, close: 100.0, volume: 100.0 },
        Kline { timestamp: 2000, open: 100.0, high: 100.5, low: 98.0, close: 99.0, volume: 100.0 },
        Kline { timestamp: 3000, open: 99.0, high: 99.5, low: 97.0, close: 98.0, volume: 100.0 },
        // 上涨趋势，应该触发买入信号
        Kline { timestamp: 4000, open: 98.0, high: 100.0, low: 97.5, close: 99.5, volume: 100.0 },
        Kline { timestamp: 5000, open: 99.5, high: 102.0, low: 99.0, close: 101.0, volume: 100.0 },
        Kline { timestamp: 6000, open: 101.0, high: 104.0, low: 100.5, close: 103.0, volume: 100.0 },
        // 下跌趋势，应该触发卖出信号
        Kline { timestamp: 7000, open: 103.0, high: 103.5, low: 101.0, close: 102.0, volume: 100.0 },
        Kline { timestamp: 8000, open: 102.0, high: 102.5, low: 100.0, close: 101.0, volume: 100.0 },
        Kline { timestamp: 9000, open: 101.0, high: 101.5, low: 99.0, close: 100.0, volume: 100.0 },
    ];
    ...
    assert_eq!(equity_curve.len(), klines.len(), "权益曲线应该有每个K线对应的点");
...
async fn test_concurrent_backtests() -> Result<()> {
    ...
    for i in 0..3 {
        ...
        let task = tokio::spawn(async move {
            run_backtest(&file_clone, "ma-crossover", 5, 10, 10000.0 + (i as f64 * 1000.0)).await
        });
        tasks.push(task);
    }
