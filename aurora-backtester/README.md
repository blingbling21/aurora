...
### 🛡️ 风险管理
- **动态止损止盈** - 基于入场价格的百分比止损止盈
  - 买入时自动设置止损止盈价格
  - 相对于实际入场价格动态计算
  - 止损价格 = 入场价 × (1 - stop_loss_pct/100)
  - 止盈价格 = 入场价 × (1 + take_profit_pct/100)
  - 卖出时自动清除止损止盈
- 最大回撤限制
- 连续亏损保护
- 单笔交易风险控制
...
```bash
# 使用配置文件运行回测（推荐方式）
aurora-backtester --config examples/backtest_config.toml

# 使用 BidAsk 定价模式
aurora-backtester --config examples/backtest_bidask_config.toml
...
```
INFO  开始回测: 数据文件=btc_1h.csv, 策略=ma-crossover, 参数=10:30
INFO  加载数据文件: btc_1h.csv
INFO  成功加载 1000 条K线数据
INFO  初始化回测引擎，策略: ma-crossover, 参数: 10:30, 初始资金: 10000.00, 定价模式: Close
...
1. **买入时自动设置**：
   - 止损价格 = 入场价 × (1 - stop_loss_pct/100)
   - 止盈价格 = 入场价 × (1 + take_profit_pct/100)
   - 例如：以 50000 买入，止损价 = 49000，止盈价 = 52500
...
```rust
use aurora_backtester::BacktestEngine;
use aurora_config::{PortfolioConfig, RiskRulesConfig};

let portfolio_config = PortfolioConfig {
    initial_cash: 10000.0,
    commission: 0.001,
    risk_rules: Some(RiskRulesConfig {
        stop_loss_pct: Some(2.0),      // 止损 2%
        take_profit_pct: Some(5.0),    // 止盈 5%
        ..Default::default()
    }),
    ..Default::default()
};
...
```bash
# 推荐使用配置文件
aurora-backtester --config <CONFIG_FILE>
...
A: 回测默认使用简化模式。建议：
- 使用 **BidAsk 定价模式** 而非 Close 模式
- 设置合理的价差（例如 0.1%）
- 配置手续费和滑点（例如手续费 0.1%，滑点 0.05%）
- 启用 **动态止损止盈** 模拟风险控制
- 在实盘前进行模拟盘测试
...
- **aurora-portfolio**: 投资组合管理（包含风险管理和仓位管理）
- **aurora-config**: 配置文件管理（支持定价模式和止损止盈配置）
- **aurora-live**: 实盘交易引擎
...
✅ **配置文件增强**
  - 新增 `PricingModeConfig` 枚举
  - 新增 `stop_loss_pct` 和 `take_profit_pct` 配置
  - 提供多个配置文件示例
