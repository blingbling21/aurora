...
- **`aurora-core`** (库) - 核心数据结构和trait定义，统一接口抽象
- **`aurora-data`** (二进制) - 数据采集工具，支持历史数据下载和实时数据流
- **`aurora-indicators`** (库) - 技术指标计算库，提供MA等常用指标
- **`aurora-strategy`** (库) - 交易策略实现，包含MA交叉等经典策略
- **`aurora-portfolio`** (库) - 投资组合管理，支持权益跟踪和风险分析
- **`aurora-backtester`** (二进制) - 历史回测引擎，完整的策略验证工具
- **`aurora-live`** (二进制) - 实时模拟交易引擎，7x24小时运行支持
...
- **模拟交易**: 所有实时交易功能均为**模拟交易**，不会执行真实订单
- **资金安全**: 框架不涉及任何真实资金操作，完全安全
- **数据来源**: 当前支持Binance公开API，数据准确性依赖于数据源
- **策略风险**: 建议在生产环境使用前进行充分的回测验证
