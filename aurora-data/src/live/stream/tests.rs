...
fn test_binance_live_stream_with_config() {
    let config = DataSourceConfig::new("wss://stream.binance.com:9443").with_timeout(60);
...
fn test_multiple_stream_instances() {
    // 测试创建多个流实例
    let streams: Vec<BinanceLiveStream> = (0..10).map(|_| BinanceLiveStream::new()).collect();
...
fn test_memory_efficiency() {
    // 测试内存使用效率
    let streams: Vec<BinanceLiveStream> = (0..50).map(|_| BinanceLiveStream::new()).collect();
