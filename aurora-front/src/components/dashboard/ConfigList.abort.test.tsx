...
describe('ConfigList - AbortController 测试', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('组件卸载时应该取消正在进行的请求', async () => {
    ...
    (configApi.list as jest.Mock).mockImplementation(() => {
      // 模拟检查 AbortSignal
      return mockListPromise;
    });
  ...
  it('refreshTrigger 变化时应该取消之前的请求', async () => {
    ...
    // 更新 refreshTrigger
    rerender(<ConfigList refreshTrigger={1} />);
  ...
  it('手动刷新不应该被 AbortController 影响', async () => {
    ...
    const { getByText } = render(<ConfigList />);
